// CougarControlV3.ino
#include <chrono> // Added to debug sluggishness
...
// Add these as global variables // Added to debug sluggishness
uint32_t loopStartTime = 0;
uint32_t loopCounter = 0;
uint32_t lastPrintTime = 0;
uint32_t maxLoopTime = 0;
uint32_t minLoopTime = 9999;
...
// Added to debug sluggishness
void printPerformanceStats() {
  uint32_t currentTime = millis();
  if (currentTime - lastPrintTime >= 1000) { // Print every second
    avgLoopTime = loopCounter > 0 ? avgLoopTime / loopCounter : 0;
    
    Serial.printf("Performance Stats - Max: %dms, Min: %dms, Avg: %dms, FPS: %.1f\n",
                 maxLoopTime, minLoopTime, avgLoopTime, 
                 1000.0 / (avgLoopTime > 0 ? avgLoopTime : 1));
...
void loop() {
  ...
  static uint32_t lvglTimer = 0;
  uint32_t currentTime = millis();

  // Manual tick handling for LVGL
  if (currentTime - lvglTimer >= 5) {  // Update LVGL every 5ms
    lv_timer_handler();
    lvglTimer = currentTime;
  }

  // Throttle input reads to 20Hz (every 50ms)
  ...
  // Update UI only on state changes
  ...
  switch (currentState) {
    case SYSTEM_SHUTDOWN:
      ...
      // DISPLAY / HIDE TEXT
      lv_label_set_text(ui_PSUDCVoltLabel, "___");                                                                // Display BLANK Value when no AC Power
      lv_label_set_text(ui_PSUDCAMPLabel, "___");                                                                 // Display BLANK Value when no AC Power
      lv_obj_set_style_text_color(ui_InfoLabel1, lv_color_hex(0xFF0000), LV_PART_MAIN | LV_STATE_DEFAULT );       // Set Info Label 1 Red     
      lv_label_set_text(ui_InfoLabel1, "SHUTDOWN");                                                               // Set Info Label 1 text To Shutdown
    ...
    case AC_POWER_STARTED:
      ...
      lv_obj_clear_flag(ui_MainContactorIndicatorBarON, LV_OBJ_FLAG_HIDDEN);                                        // SHOW the white ON indicator on The Main Contctor
      // DISPLAY / HIDE TEXT
      lv_label_set_text(ui_PSUDCVoltLabel, "___");                                                                // Display BLANK Value when AC Power But Master DC OFF
      lv_label_set_text(ui_PSUDCAMPLabel, "___");                                                                 // Display BLANK Value when AC Power But Master DC OFF
      lv_obj_set_style_text_color(ui_InfoLabel1, lv_color_hex(0x00FF00), LV_PART_MAIN | LV_STATE_DEFAULT );       // Set Info Label 1 Green     
      lv_label_set_text(ui_InfoLabel1, "STARTED");                                                               // Set Info Label 1 text To Started
      lv_label_set_text(ui_VehicleLiveLabel,  ""); 
    ...
    case AC_SURFACE_ON:
      ...
      lv_label_set_text(ui_PSUDCAMPLabel, "___");                                                                 // Display BLANK Value when AC Power But Master DC OFF
      lv_obj_set_style_text_color(ui_InfoLabel1, lv_color_hex(0x00FF00), LV_PART_MAIN | LV_STATE_DEFAULT );       // Set Info Label 1 Green     
      lv_label_set_text(ui_InfoLabel1, "AC Surface");
      lv_label_set_text(ui_VehicleLiveLabel,  "");                                                              
    ...
    case AC_VEHICLE_ON:
      ...
      lv_label_set_text(ui_PSUDCAMPLabel, "___");                                                                 // Display BLANK Value when AC Power But Master DC OFF
      lv_obj_set_style_text_color(ui_InfoLabel1, lv_color_hex(0x00FF00), LV_PART_MAIN | LV_STATE_DEFAULT );       // Set Info Label 1 Green     
      lv_label_set_text(ui_InfoLabel1, "AC Vehicle");                                                              // Set Info Label 1 text To Started
      lv_obj_set_style_text_color(ui_VehicleLiveLabel, lv_color_hex(0xFF0000), LV_PART_MAIN | LV_STATE_DEFAULT );  // Set VehicleLiveLabel Red     
      lv_label_set_text(ui_VehicleLiveLabel, "VEHICLE LIVE");                                                            // Set VehicleLiveLabel To VEHICLE LIVE
...
// ENCODER INDICATOR LED SETTINGS
      ...
      lv_obj_clear_flag(ui_MainContactorIndicatorBarON, LV_OBJ_FLAG_HIDDEN);                                        // SHOW the white ON indicator on The Main Contctor
      // DISPLAY / HIDE TEXT
      lv_label_set_text(ui_PSUDCVoltLabel, "502");                                                                  // Display Some Dummy Voltage Value ON PSU when Master DC ON
      lv_label_set_text(ui_PSUDCAMPLabel, "___");                                                                 // Display BLANK Value when Master DC ON - But Thrusters Diabled
      lv_obj_set_style_text_color(ui_InfoLabel1, lv_color_hex(0x00FF00), LV_PART_MAIN | LV_STATE_DEFAULT );       // Set Info Label 1 Green     
      lv_label_set_text(ui_InfoLabel1, "AC Vehicle");                                                              // Set Info Label 1 text To Started
      lv_obj_set_style_text_color(ui_VehicleLiveLabel, lv_color_hex(0xFF0000), LV_PART_MAIN | LV_STATE_DEFAULT );  // Set VehicleLiveLabel Red     
      lv_label_set_text(ui_VehicleLiveLabel, "VEHICLE LIVE");                                                            // Set VehicleLiveLabel To VEHICLE LIVE
    ...
    case THRUSTERS_ENABLED_DC1_ONLY:
      ...
      // DISPLAY / HIDE TEXT
      lv_label_set_text(ui_PSUDCVoltLabel, "502");                                                                  // Display Some Dummy Voltage Value ON PSU when Master DC ON
      lv_label_set_text(ui_PSUDCAMPLabel, "04.2");                                                                  // Display Dummy AMPS when only 1 DC On & Thruster Enabled
      lv_obj_set_style_text_color(ui_InfoLabel1, lv_color_hex(0x00FF00), LV_PART_MAIN | LV_STATE_DEFAULT );       // Set Info Label 1 Green     
      lv_label_set_text(ui_InfoLabel1, "AC Vehicle");                                                              // Set Info Label 1 text To Started
      lv_obj_set_style_text_color(ui_VehicleLiveLabel, lv_color_hex(0xFF0000), LV_PART_MAIN | LV_STATE_DEFAULT );  // Set VehicleLiveLabel Red     
      lv_label_set_text(ui_VehicleLiveLabel, "VEHICLE LIVE");                                                            // Set VehicleLiveLabel To VEHICLE LIVE
    ...
    case THRUSTERS_ENABLED_DC2_ONLY:
      ...
      // DISPLAY / HIDE TEXT
      lv_label_set_text(ui_PSUDCVoltLabel, "502");                                                                  // Display Some Dummy Voltage Value ON PSU when Master DC ON
      lv_label_set_text(ui_PSUDCAMPLabel, "03.3");                                                                  // Display Dummy AMPS when only 1 DC On & Thruster Enabled
      lv_obj_set_style_text_color(ui_InfoLabel1, lv_color_hex(0x00FF00), LV_PART_MAIN | LV_STATE_DEFAULT );       // Set Info Label 1 Green     
      lv_label_set_text(ui_InfoLabel1, "AC Vehicle");                                                              // Set Info Label 1 text To Started
      lv_obj_set_style_text_color(ui_VehicleLiveLabel, lv_color_hex(0xFF0000), LV_PART_MAIN | LV_STATE_DEFAULT );  // Set VehicleLiveLabel Red     
      lv_label_set_text(ui_VehicleLiveLabel, "VEHICLE LIVE");                                                            // Set VehicleLiveLabel To VEHICLE LIVE
...
// ENCODER INDICATOR LED SETTINGS
      ...
      // DISPLAY / HIDE TEXT
      lv_label_set_text(ui_PSUDCVoltLabel, "502");                                                                  // Display Some Dummy Voltage Value ON PSU when Master DC ON
      lv_label_set_text(ui_PSUDCAMPLabel, "10.1");                                                                  // Display Dummy AMPS when both Thrusters Enabled
      lv_obj_set_style_text_color(ui_InfoLabel1, lv_color_hex(0x00FF00), LV_PART_MAIN | LV_STATE_DEFAULT );       // Set Info Label 1 Green     
      lv_label_set_text(ui_InfoLabel1, "SYSTEM READY");                                                              // Set Info Label 1 text To Started
      lv_obj_set_style_text_color(ui_VehicleLiveLabel, lv_color_hex(0xFF0000), LV_PART_MAIN | LV_STATE_DEFAULT );  // Set VehicleLiveLabel Red     
      lv_label_set_text(ui_VehicleLiveLabel, "VEHICLE LIVE");                                                            // Set VehicleLiveLabel To VEHICLE LIVE
  ...
  // Performance monitoring (keep your existing code)
  ...
  // Reduced delay with yield() to maintain responsiveness
  delay(2);  // Reduced from 5ms
}
